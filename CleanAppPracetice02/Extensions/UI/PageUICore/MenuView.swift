...
open class MenuView: UIScrollView {
    ...
    fileprivate var seperateViewArray = [UIView]()
    fileprivate func setupSeperateViewIfNeeded() {
        
        guard case .noneSeparate = menuOptions.focusMode else { return }
        
        seperateViewArray = [UIView]()
        var p:CGFloat = 0.0
        for miv in menuItemViews {
            if p == 0 {
                p += 1.0
                continue
            }
            let seperateView = UIView()
            seperateView.frame = CGRect(x: (miv.bounds.width * p) , y: 0, width: 1, height: miv.bounds.height)
            seperateView.backgroundColor = UIColor.white
            seperateViewArray.append(seperateView)
    ...
    fileprivate func setupRoundRectViewIfNeeded() {
        guard case let .roundRect(radius, _, verticalPadding, selectedColor,isCircle) = menuOptions.focusMode else { return }
        
        let height = menuOptions.height - verticalPadding * 2
        roundRectView.frame = CGRect(x: 0, y: verticalPadding, width: 0, height: height)
        if isCircle{
            roundRectView.layer.cornerRadius = height / 2
        }else{
    ...
    fileprivate func animateRoundRectViewIfNeeded() {
        guard case .roundRect(_, let horizontalPadding, _, _, _) = menuOptions.focusMode else { return }
...
extension MenuView {
    func cleanup() {
        ...
        case .noneSeparate:
            for iView in seperateViewArray {
